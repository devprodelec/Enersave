<div class="half">  
  <div class="header">
	<h3><?php echo __("Editar Proceso") ?></h3>
	<?php
	  $options = array('class' => 'icon list');
	  echo $this->Html->link('Detalles', '/processes/'.$process_id, $options);
	?>
  </div>
  <?php
	$inputDefaults = array('label'=>false, 'error'=>array('wrap'=>'span', 'class'=>'error')); 
	echo $this->Form->create('Process', array('action' => 'edit', 'inputDefaults' => $inputDefaults));
